...
**Goal:** Redesign the calendar page to show all matches grouped by date by default, fix search, add a date picker, dynamic quick-filter buttons, flag emoji badges, and title-case league names.

**Architecture:** Server-side fetch via tRPC (Strapi token stays server-only), React Query hydrates the client. All filtering (date, search) is 100% client-side. `react-hook-form` controls the search input. `shadcn` Calendar + Popover form the date picker.

**Tech Stack:** Next.js 16, tRPC 11, React Query 5, react-hook-form, shadcn (base-nova style), date-fns + date-fns-tz, Tailwind v4.

---

### Task 1: Install react-hook-form and shadcn Calendar + Popover
...
Expected: `react-hook-form` appears in `package.json` dependencies.

- [ ] **Step 2: Add shadcn Calendar and Popover**

```bash
cd frontend && npx shadcn@latest add calendar popover
```

Expected: `src/components/ui/calendar.tsx` and `src/components/ui/popover.tsx` created, `react-day-picker` added to package.json.

- [ ] **Step 3: Verify Calendar renders**

Check that `src/components/ui/calendar.tsx` exists and imports from `react-day-picker`.

```bash
head -5 src/components/ui/calendar.tsx
```

Expected: file starts with imports from `react-day-picker`.

- [ ] **Step 4: Commit**

```bash
git add package.json package-lock.json src/components/ui/calendar.tsx src/components/ui/popover.tsx
git commit -m "feat: add react-hook-form, shadcn calendar and popover"
```

---

### Task 2: Fix `fetchMatchPicks` pagination and wire calendar page to `getAll`

**Files:**
- Modify: `src/lib/strapi.ts` — add `pagination[pageSize]: '100'`
- Modify: `src/app/calendario/page.tsx` — prefetch `getAll` instead of `getUpcoming`

- [ ] **Step 1: Add pageSize to fetchMatchPicks**

In `src/lib/strapi.ts`, update `fetchMatchPicks()` to add pageSize:
...
- [ ] **Step 2: Update calendar page to prefetch getAll**

Replace the prefetchQuery call in `src/app/calendario/page.tsx`:
...
```bash
git add src/lib/strapi.ts src/app/calendario/page.tsx
git commit -m "fix: calendar prefetches all matches with correct pageSize"
```

---

### Task 3: Expand tournament map with flag emojis and fix league display

**Files:**
- Modify: `src/lib/format.ts` — add `TOURNAMENT_FLAGS` map and `getTournamentFlag()` export
- Note: `TOURNAMENT_LABELS` already has title-case names — no change needed there

- [ ] **Step 1: Add flag map and helper to format.ts**

Add after the existing `TOURNAMENT_LABELS` constant in `src/lib/format.ts`:
...
git commit -m "feat: add tournament flag emoji map"
```

---

### Task 4: Rewrite CalendarView component

**Files:**
- Modify: `src/components/calendar-view.tsx` — full rewrite with new grouping, search via react-hook-form, date picker, dynamic buttons, flag badges, title-case names

This is the main task. Replace the entire file content with the implementation below.

- [ ] **Step 1: Replace calendar-view.tsx**

Write `src/components/calendar-view.tsx` with this content:
...
git commit -m "feat: redesign calendar — temporal grouping, date picker, flag badges, react-hook-form search"
```
...
- [ ] **Step 2: Open browser at http://localhost:3000/calendario**

Check:
- All upcoming matches appear on load (no day pre-selected)
- Matches grouped by date blocks ("Hoje", "Amanhã", etc.)
- Within each date, grouped by league with flag emoji badge
- League names in title case (not UPPERCASE)
- Up to 4 quick-filter day buttons visible, only for days with matches
- Quick-filter buttons filter the list; clicking again clears filter
- "Outra data" date picker opens a calendar; selecting a date filters list
- X button clears date filter and shows all again
- Search input (sidebar) filters across all matches regardless of date
- Combining search + date filter narrows results correctly
...
**Spec coverage:**
- ✅ All matches shown by default, grouped by date — Task 4 (selectedDate starts as null, grouped Map has all dates)
- ✅ Search works across all matches — Task 4 (search filter doesn't gate on date)
- ✅ Date picker (shadcn Calendar + Popover) at end of quick-filter row — Task 4
- ✅ Dynamic quick-filter buttons (up to 4, days with matches only) — Task 4 (`datesWithMatches.slice(0, 4)`)
- ✅ League names title case — Task 4 (removed `.toUpperCase()` from header render)
- ✅ Flag emoji badges — Task 3 + Task 4 (`getTournamentFlag()`)
- ✅ react-hook-form for search input — Task 4 (`useForm`, `register("search")`, `watch("search")`)
- ✅ Server-side fetch only (Strapi token never sent to client) — Task 2 (tRPC server procedure, no change to strapi.ts auth)
- ✅ Pagination fix for getAll — Task 2

**Placeholder scan:** None found — all steps include actual code.

**Type consistency:**
- `getTournamentFlag` defined in Task 3, used in Task 4 ✅
- `SearchForm` type defined and used within Task 4 ✅
- `trpc.matchPicks.getAll.queryOptions()` — `getAll` exists in router ✅
